#!/usr/bin/env python3
"""Run infilling model benchmarking on Modal.

Thin wrapper around ``scripts/benchmark_infilling_models.py`` that handles
...
APP_NAME = "leandojo-benchmark-infilling-models"
WORKSPACE_DIR = "/workspace"
CACHE_ROOT = "/cache"
OUTPUT_ROOT = "/outputs"
MODEL_FLAGS = {"--diffusion-model", "--ar-model"}
...
def _normalize_benchmark_args(raw_args: list[str]) -> list[str]:
    ...
    if not _flag_has_value(normalized, "--json-output"):
        normalized.extend(
            [
                "--json-output",
                f"{OUTPUT_ROOT}/benchmarks/benchmark_infilling_models_{_timestamp()}.json",
            ]
        )
    if not _flag_has_value(normalized, "--csv-output"):
        normalized.extend(
            [
                "--csv-output",
                f"{OUTPUT_ROOT}/benchmarks/benchmark_infilling_models_{_timestamp()}.csv",
            ]
        )
    return normalized
...
def _build_command(forwarded_args: list[str]) -> list[str]:
    return ["uv", "run", "python", "scripts/benchmark_infilling_models.py", *forwarded_args]
...
def run_benchmark(benchmark_args: list[str]) -> None:
    env = os.environ.copy()
    ...
    normalized_args = _normalize_benchmark_args(benchmark_args)
    cmd = _build_command(normalized_args)

    print("Running on Modal GPU: 1x H100")
    print("Launching benchmark command:")
    print(" ".join(shlex.quote(part) for part in cmd))
    subprocess.run(cmd, cwd=WORKSPACE_DIR, env=env, check=True)


@app.local_entrypoint()
def main(benchmark_args: str = "") -> None:
    forwarded_args = shlex.split(benchmark_args)
    run_benchmark.remote(forwarded_args)


def cli_main() -> None:
    parser = argparse.ArgumentParser(
        description="Run infilling model benchmarking on Modal.",
    )
    parser.add_argument(
        "benchmark_args",
        nargs=argparse.REMAINDER,
        help="Arguments forwarded to scripts/benchmark_infilling_models.py.",
    )
    args = parser.parse_args()

    forwarded_args = list(args.benchmark_args)
